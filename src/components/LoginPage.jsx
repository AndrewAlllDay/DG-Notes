        ...
        try {
            if (isRegistering) {
                ...
                await setUserProfile(uid, {
                    ...
                    email: email.trim(),
                    role: 'player' // Changed default role from 'non-player' to 'player'
                });
            ...
            console.error("DEBUG LoginPage: Authentication error:", err.code, err.message, err);
            switch (err.code) {
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            ...
            if (!existingProfile) {
                ...
                // Note: The role is NOT explicitly updated here, it retains its existing role.
            ...
            console.error("DEBUG LoginPage: Error during Google Sign-In:", err.code, err.message, err);
            setError('Google Sign-In failed. Please try again.');
        }
    };

    if (user && isAuthReady) {
        return null; // If user is logged in and auth is ready, render nothing (App.jsx will handle routing)
    }

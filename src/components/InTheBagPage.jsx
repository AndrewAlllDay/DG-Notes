...
export default function InTheBagPage({ user: currentUser }) {
    ...
    // ✨ Changed state to track a single open accordion (string or null)
    ...
    // ✨ Updated toggle logic to handle a single open state
    ...
    const handleDetailsSubmit = useCallback(async (detailsData) => { /* Full logic remains the same */ }, [currentUser, currentDiscToEdit, pendingApiDisc, activeDiscs, archivedDiscs]);
    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-black text-gray-900 dark:text-gray-100 p-4 sm:p-6 lg:p-8 pb-48">
                    ...
                    <div className="space-y-4">
                        {groupedAndSortedDiscs.sortedTypes.map(type => (
                            <Accordion key={type} title={<span className='text-black dark:text-blue-400 text-xl'>{type} <span className='text-black dark:text-white text-base font-light'>({groupedAndSortedDiscs.grouped[type].length} discs)</span></span>}
                                // ✨ Updated isOpen prop to a simple comparison
                            ...
                            >
                                <motion.ul className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2" variants={listVariants} initial="hidden" animate="visible">
                                    {groupedAndSortedDiscs.grouped[type].map(disc => (
                                        <motion.li key={disc.id} variants={itemVariants} className="self-stretch">
            ...
            {archivedDiscs.length > 0 && (
                <>
                    ...
                    <Accordion title={`On the Shelf (${archivedDiscs.length} discs)`}
                        // ✨ Updated isOpen prop for the archived section
                    ...
                    >
                        <motion.ul className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2" variants={listVariants} initial="hidden" animate="visible">
                            {archivedDiscs.sort((a, b) => (a.name || '').localeCompare(b.name || '')).map(disc => (
                                <motion.li key={disc.id} variants={itemVariants} className="self-stretch">
                                    <Tilt glareEnable={true} glareMaxOpacity={0.05} glarePosition="all" tiltMaxAngleX={8} tiltMaxAngleY={8} scale={1.03} className="h-full">
                                        <div className="h-full"><DiscItem disc={disc} type="archived" isExpanded={expandedDiscId === disc.id} onToggleExpand={handleToggleExpand} onRestore={handleRestoreDisc} onDelete={handleDeleteDisc} /></div>
                                    </Tilt>
                                </motion.li>
                            ))}
                        </motion.ul>
            ...
            <DiscFormModal isOpen={isDetailsModalOpen} onClose={() => { setIsDetailsModalOpen(false); setCurrentDiscToEdit(null); setPendingApiDisc(null); }} onSubmit={handleDetailsSubmit} initialData={currentDiscToEdit || pendingApiDisc} discTypes={dynamicDiscTypes} />
            <DeleteConfirmationModal isOpen={deleteModalState.isOpen} onClose={cancelDeleteDisc} onConfirm={confirmDeleteDisc} message={`Are you sure you want to permanently delete '${deleteModalState.disc?.name}'? This cannot be undone.`} />
        </div>
    );
}
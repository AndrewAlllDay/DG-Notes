...
export default function CourseList({ courses, onDeleteCourse, onSelectCourse, tournamentName }) {
    ...
    console.log(`[CourseList] Current swipedCourseId: ${swipedCourseId}`);
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            if (swipedCourseId && listRef.current && !listRef.current.contains(event.target)) {
                console.log(`[CourseList] Clicked outside current swiped item (${swipedCourseId}). Closing.`);
    ...
    const handleTouchStart = (e, id) => {
        ...
        if (swipedCourseId && swipedCourseId !== id) {
            setSwipedCourseId(null);
            console.log(`[CourseList] Closing previous swiped item ${swipedCourseId} before new swipe on ${id}`);
        }

        touchStartX.current = e.touches[0].clientX;
        swipingItemId.current = id;
        hasSwipedEnough.current = false; // Reset for the new swipe
        console.log(`[CourseList] Touch Start on ${id}. Initial X: ${touchStartX.current}`);
    };
    ...
    const handleTouchMove = (e, id) => {
        ...
        console.log(`[CourseList] Touch Move on ${id}. Diff: ${diff}`);
        ...
        if (diff < -40 && !hasSwipedEnough.current) {
            console.log(`[CourseList] Swiped enough to reveal for ${id}. Setting swipedCourseId to ${id}`);
        ...
        if (swipedCourseId === id && diff > 20) { // If it's the currently swiped item AND swiping right past threshold
            console.log(`[CourseList] Swiped right enough to close for ${id}. Setting swipedCourseId to null`);
            setSwipedCourseId(null);
    ...
    const handleTouchEnd = (id) => {
        console.log(`[CourseList] Touch End for ${id}. Final swipedCourseId: ${swipedCourseId}. hasSwipedEnough: ${hasSwipedEnough.current}`);
        ...
        if (swipedCourseId === id && hasSwipedEnough.current) {
            console.log(`[CourseList] Item ${id} remains swiped open.`);
        ...
        // In this scenario, we want to close the item.
            ...
            console.log(`[CourseList] Item ${id} was open or not fully swiped. Closing.`);
            setSwipedCourseId(null);
        }
